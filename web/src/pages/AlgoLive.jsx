      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-xl text-text-primary">{algoState?.strategy || 'Algo'} — {algoState?.symbol || 'N/A'}</h1>
        ...
        <button onClick={handleStop} disabled={stopping} className="flex items-center justify-center gap-2 px-6 py-3 bg-pink hover:bg-pink/90 text-white rounded-lg text-sm transition-colors disabled:opacity-50 cursor-pointer">
          {stopping ? <Loader2 size={18} className="animate-spin" /> : <Square size={18} />}
          {stopping ? 'Stopping...' : 'STOP'}
        </button>
      </div>

      {/* === TWO COLUMNS: Metrics LEFT | Logs RIGHT === */}
      <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-3">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-bg-card border border-border-default rounded-lg p-5 text-center">
            ...
            <p className={`text-4xl font-mono-nums transition-colors ${pnl != null ? (pnlVal >= 0 ? 'text-profit' : 'text-loss') : 'text-text-dim'} ${pnl != null ? 'animate-pulse-glow' : ''}`}>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-bg-card border border-border-default rounded-lg p-2.5 text-center">{posIcon()}<p className="text-[10px] text-text-muted mb-0.5">Position</p><p className={`text-xs font-mono-nums ${posColor}`}>{position}</p></div>
        ...
        <div className="bg-[#0a0a18] border border-border-default rounded-lg overflow-hidden flex flex-col" style={{ minHeight: '520px' }}>
          ...
          <div ref={logRef} className="p-3 space-y-0.5 overflow-y-auto font-mono text-xs leading-relaxed flex-1">
            {events.length === 0 ? (
...
function Stat({ icon: Icon, iconCls = 'text-accent', label, value, cls = 'text-text-primary', mono, truncate }) {
  return (
    <div className="bg-bg-card border border-border-default rounded-lg p-2.5 text-center">
      ...
      <p className={`text-xs ${cls} ${mono ? 'font-mono-nums' : ''} ${truncate ? 'truncate' : ''}`}>{value}</p>
    </div>
  );
}
      ...
      <div className="bg-bg-card border border-border-default rounded-lg overflow-hidden">
        ...
        <div className="border-t border-border-default grid grid-cols-2 divide-x divide-border-default">
          ...
          <div className="p-4 text-center">
            ...
            <p className={`text-lg font-mono-nums ${wc}`}>{summary.winRate != null ? `${summary.winRate.toFixed(1)}%` : 'N/A'}</p>
          </div>
        </div>
        <div className="border-t border-border-default grid grid-cols-2 divide-x divide-border-default">
          <div className="p-4 text-center">
